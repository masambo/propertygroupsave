...
filterButtons.forEach((button) => {
  button.addEventListener("click", () => {
    ...
    propertyCards.forEach((card) => {
      card.hidden = filter !== "all" && card.dataset.bedrooms !== filter;
    });
  });
});
...
enquiryForm.addEventListener("submit", (event) => {
  event.preventDefault();
  enquiryForm.reset();
  formMessage.textContent = "Thank you for contacting Property Group Save Namibia. Our team has received your enquiry and will contact you with more information about available property options.";
});
